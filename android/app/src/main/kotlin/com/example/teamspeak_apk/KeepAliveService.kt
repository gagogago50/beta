...
class KeepAliveService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        // The wake lock is created here but taken only while audio actually
        // needs the CPU (see [applyWakeLock]). Holding a PARTIAL_WAKE_LOCK for
        // the whole session — including hours of fully muted idling — is the
        // single most expensive thing a background app can do.
        val pm = getSystemService(Context.POWER_SERVICE) as PowerManager
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "teamspeak:keepalive")
        applyWakeLock(needed = true)
    ...
    /**
     * Takes or releases the partial wake lock.
     *
     * Audio playback keeps the CPU awake by itself through the audio stream;
     * the lock is what protects the *engine* threads (jitter buffer, decoding,
     * keep-alive packets) from Doze. When the user is fully muted — no capture
     * and no playback — none of that needs to run at high frequency, so the
     * lock is dropped and the foreground service alone keeps the process
     * alive. Unmuting takes it back immediately.
     */
    private fun applyWakeLock(needed: Boolean) {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        // Fully muted means neither capture nor playback: let the CPU sleep
        // between packets instead of pinning it awake.
        applyWakeLock(needed = !fullMuted)
